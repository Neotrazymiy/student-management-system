...
CREATE TABLE IF NOT EXISTS s.faculty (
	...
	CONSTRAINT fk_facity_university FOREIGN KEY (university_id) REFERENCES s.university (id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS s.department (
	...
	CONSTRAINT fk_department_faculty FOREIGN KEY (faculty_id) REFERENCES s.faculty (id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS s.groupe (
	...
	name VARCHAR(50) UNIQUE NOT NULL,
	department_id UUID,
	CONSTRAINT fk_group_department FOREIGN KEY (department_id) REFERENCES s.department (id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS s.course (
	...
	department_id UUID,
	CONSTRAINT fk_course_department FOREIGN KEY (department_id) REFERENCES s.department (id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS s.lesson (
	...
	CONSTRAINT fk_lesson_room FOREIGN KEY (room_id) REFERENCES s.room (id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS s.student (
	...
	groupe_id UUID,
	CONSTRAINT fk_student_user FOREIGN KEY (user_id) REFERENCES s.userr (id) ON DELETE CASCADE,
	CONSTRAINT fk_student_groupe FOREIGN KEY (groupe_id) REFERENCES s.groupe (id) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS s.teacher (
	...
	user_id UUID UNIQUE NOT NULL,
	department_id UUID,
	CONSTRAINT fk_teacher_user FOREIGN KEY (user_id) REFERENCES s.userr (id) ON DELETE CASCADE,
	CONSTRAINT fk_teacher_department FOREIGN KEY (department_id) REFERENCES s.department (id) ON DELETE CASCADE
);
