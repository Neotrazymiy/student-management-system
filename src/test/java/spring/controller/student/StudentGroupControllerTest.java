...
@WebMvcTest(controllers = StudentGroupController.class, excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = SecurityConfig.class))
class StudentGroupControllerTest {
	...
	void getPageGroupTest() throws Exception {
		...
		mockMvc.perform(get("/student/groups")).andExpect(status().isOk())
				.andExpect(view().name("student/groups/groups")).andExpect(model().attributeExists("pageGroup"))
				.andExpect(model().attribute("currentPage", 0)).andExpect(model().attribute("pageSize", 10))
				.andExpect(model().attribute("totalPage", 1))
				.andExpect(model().attribute("basePath", "/student/groups"));
	}
