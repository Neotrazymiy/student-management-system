...
class MethodistStudentControllerTest {
	...
	void getPageStudent() throws Exception {
		...
		mockMvc.perform(get("/methodist/students").param("groupPage", "0").param("groupSize", "5").param("groupId",
				groupId.toString())).andExpect(status().isOk()).andExpect(view().name("methodist/students/students"))
				...
				.andExpect(model().attribute("basePath", "/methodist/students"));
	}
