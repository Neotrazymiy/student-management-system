...
@WebMvcTest(controllers = TeacherCourseController.class, excludeFilters = @ComponentScan.Filter(type = FilterType.ASSIGNABLE_TYPE, classes = SecurityConfig.class))
class TeacherCourseControllerTest {
	...
	void getTeacherTest() throws Exception {
		...
		mockMvc.perform(get("/teacher/courses")).andExpect(status().isOk())
				.andExpect(view().name("teacher/courses/courses"))
